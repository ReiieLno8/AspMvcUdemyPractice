...
{
    [Area("Admin")] // Tells the controller that this is belong to a specific area
    public class CategoryController : Controller
    {
        private readonly IUnitOfWork _unitOfWork; //check Udemy form explanation
        ...
        public IActionResult Edit(int? id) // by default http is automatically Get so no need to include [httpsGet]
        {
            ...
            //Category? categoryFromDb = _categoryRepo.Categories.Find(id); // Find() only works on primary key
            //Category? category1 =  _Categories.FirstOrDefault(i => i.Id == id); different kinds of filtering 
            //Category? category2 =  _Categories.Where(i => i.Id == id).FirstOrDefault(); different kinds of filtering 
        ...
        public IActionResult Delete(int? id) // by default http is automatically Get so no need to include [httpsGet]
        {
            ...
            Category? categoryFromDb = _unitOfWork.Category.Get(u => u.Id == id);
            //Category? categoryFromDb = _categoryRepo.Find(id); Find() only works on primary key
            //Category? category1 = _categoryRepo.Categories.FirstOrDefault(i => i.Id == id); different kinds of filtering 
            //Category? category2 = _categoryRepo.Categories.Where(i => i.Id == id).FirstOrDefault(); different kinds of filtering 
            if (categoryFromDb == null)
            {
                return NotFound();
            }
